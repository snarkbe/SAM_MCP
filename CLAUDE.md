...
**`sam_mcp.server`** — FastMCP server. All tools open the DB read-only (`file:...?mode=ro`). Diagnostics (startup counts, errors) go to **stderr** — stdout is the JSON-RPC channel in stdio mode. In `--http` mode, FastMCP's built-in DNS-rebinding protection is explicitly disabled (the proxy/LAN is the trust boundary); `--allowed-hosts` is the opt-in replacement. Also in `--http` mode only, `_install_log_capture()` tees stdout/stderr into a ring buffer served at `GET /log` (what `docker logs` shows); it must run before `uvicorn.run` (uvicorn resolves `ext://sys.stdout|stderr` at dictConfig time) and it re-points the root logging handler, which FastMCP bound to the pre-tee stderr via `logging.basicConfig` in its constructor at import time.
